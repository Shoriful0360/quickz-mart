...
useEffect(() => {
  if (Singleproduct && !Singleproduct.error) {
    ...
    // localStorage-এর product কার্টে আছে কিনা দেখো
    const inCart = carts.find(item => item.id === Singleproduct.id);


    if (inCart) {
      // Redux carts → updated quantity
      setProducts([{ ...inCart }]);
    } else {
      // নতুন add করা product → localStorage থেকে
      setProducts([{ ...Singleproduct, quantity }]);
    }
...
const handleUpdateQty = (type, id) => {
  ...
  // 🔹 Calculations
  const subtotal = products.reduce((acc, item) => acc + item.price * Number(item.quantity || 1), 0);
  ...
  // 🔹 Pass total price to parent
  useEffect(() => {
    setTotalProductPrice(totalOrder);
  }, [totalOrder, setTotalProductPrice]);
  ...
  return (
    <div className="max-w-md mx-auto p-6 rounded-xl shadow-xl bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 transition-colors">
      ...
      <div className="space-y-4">
        {products.map(item => (
          <div key={item._id || item.id} className="flex relative gap-4 items-center bg-gray-100 dark:bg-gray-800 p-3 rounded-lg shadow-inner transition-colors">
            {!buyId && (
              <button
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-600 dark:text-gray-300">Price: ৳ {(item.price || 0).toFixed(2)}</p>

              {!buyId && (
                <div className="flex items-center gap-2 mt-2">
                  <button onClick={() => decrease(item.id)} className="px-3 py-1 rounded-md bg-red-500 text-white hover:bg-red-600 transition">-</button>
                  <span className="px-3 py-1 rounded-md bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-100">{item.quantity}</span>
                  <button onClick={() => increase(item.id)} className="px-3 py-1 rounded-md bg-green-500 text-white hover:bg-green-600 transition">+</button>
                </div>
              )}
      ...
      <div className="mt-6 space-y-3 text-gray-700 dark:text-gray-300">
        ...
        {/* Delivery */}
        <div className="mt-4">
          <label className="block mb-1 font-medium text-gray-800 dark:text-gray-200">Delivery Area</label>
          <select
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Delivery Charge: ৳ {deliveryCharge.toFixed(2)}</p>
        </div>
